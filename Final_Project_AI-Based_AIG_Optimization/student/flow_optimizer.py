...
from result_logging import (
    ...
    build_case_coverage,
    count_selected_improvement_cases,
...
def _phase_structural_resynthesis(
    ...
    run_adaptive_compact_vector_pareto(
        _filter_cases(ALL_CASES), args.abc, args.benchmarks, args.output, args.logs, root, cfg.seed,
    )
...
def _phase_convergence(
    ...
    pass1_cases = sorted(
        [c for c in _filter_cases(ALL_CASES) if case_area.get(c, 0) >= cfg.my_deepsyn_pass1_min_area],
...
def _run_reproduce_modes(args: object, root: Path) -> int | None:
    """Handle --show-reproduce-recipe, --write-contest-plan, --reproduce-best.

    Returns an exit code if the mode was handled, or None to fall through.
    """
...
def _run_report_modes(args: object, root: Path) -> int | None:
    """Handle all read-only reporting / analysis modes.
...
def _run_scheduling_modes(args: object, root: Path) -> int | None:
    """Handle high-level scheduling / contest orchestration modes.
...
def _run_refinement_modes(args: object, root: Path) -> int | None:
    """Handle iterative refinement modes (type-guided, objective, micro, etc.).
    ...
    if args.circuit_type_optimize:
        ...
        for case in cases:
            ...
            _rows, summary = run_circuit_type_optimize_case(
                ...
                args.timeout_per_case,
                root,
                args.circuit_type_max_flows,
                args.circuit_type_max_seeds,
                args.seed,
            )
            summaries.append(summary)
    ...
    if args.semantic_split_optimize:
        ...
        for case in cases:
            ...
            _rows, summary = run_semantic_split_optimize_case(
                ...
                args.timeout_per_case,
                root,
                args.semantic_max_splits,
                args.semantic_max_flows,
                args.seed,
            )
            summaries.append(summary)
...
def _run_rescue_modes(args: object, root: Path) -> int | None:
    """Handle rescue / structural exploration modes.
    ...
    if args.long_large_structural:
        ...
        if not args.case and not args.range:
            ...
            for case in cases:
                table = read_truth(args.benchmarks / f"{case}.truth")
                area, _delay, adp = measure_adp(args.abc, args.output / f"{case}.aig", 120, root)
                if should_run_long_large_structural(
                    table, area, adp, args.long_large_min_area, args.long_large_min_adp
                ):
        ...
        for case in cases:
            ...
            summary = run_long_large_structural_case(
                ...
                root,
                args.seed,
                args.long_large_seconds,
                args.ttopt_seed_rounds,
            )
            summaries.append(summary)
...
def _run_default_optimize(args: object, root: Path) -> int:
    """Run the default sweep/polish/optimize loop and write final CSVs."""
    if args.case:
        cases = [args.case]
    elif args.range:
        start = int(args.range[0].removeprefix("ex"))
